...
export default function CursosPage() {
  ...
  const gerarPDF = () => {
    ...
    const nomeCurso = cursoSelecionado.nome;
    const descCurso = cursoSelecionado.descricao;
    const cargaCurso = cursoSelecionado.carga;
    const idCurso = cursoSelecionado.id;
    const totalAlunos = String(alunos.length);
    const bodyHtml = '<div id="pdf-content"><div style="text-align:center;margin-bottom:24px;"><div style="font-size:11px;color:#64748b;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:4px;">TULIPA VENDEDORAS</div><div style="font-size:22px;font-weight:900;color:#0d5e35;margin-bottom:2px;">' + nomeCurso + '</div><div style="font-size:12px;color:#64748b;">' + descCurso + '</div></div><div style="display:flex;justify-content:space-between;margin-bottom:20px;font-size:11px;color:#64748b;"><span>Data: ' + dataAtual + '</span><span>Carga: ' + cargaCurso + '</span><span>Total: ' + totalAlunos + ' aluno(s)</span></div><table style="width:100%;border-collapse:collapse;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;"><thead><tr style="background:#0d5e35;color:#fff;"><th style="padding:10px 12px;text-align:center;font-size:11px;font-weight:700;width:40px;">N&#186;</th><th style="padding:10px 12px;text-align:left;font-size:11px;font-weight:700;">NOME DO ALUNO</th><th style="padding:10px 12px;text-align:left;font-size:11px;font-weight:700;width:140px;">TELEFONE</th></tr></thead><tbody>' + rows + '</tbody></table><div style="margin-top:30px;text-align:center;font-size:10px;color:#94a3b8;"><p>Gerado em ' + dataAtual + ' - Tulipa Vendedoras</p></div></div>';
    const fileName = "lista-" + idCurso + "-" + dataAtual.replace(/\//g, "-") + ".pdf";
    const win = window.open("", "_blank");
    if (!win) return;
    const script = '<script src="https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js"><\/script>';
    const runScript = '<script>document.body.innerHTML=\'' + bodyHtml.replace(/'/g, "\\'") + '\';document.body.style.opacity="1";window.onload=function(){html2pdf().set({margin:[15,15,15,15],filename:"' + fileName + '",image:{type:"jpeg",quality:0.98},html2canvas:{scale:2,useCORS:true},jsPDF:{unit:"mm",format:"a4",orientation:"portrait"}}).from(document.getElementById("pdf-content")).save().then(function(){window.close();});};<\/script>';
    win.document.write("<html><head><title>Lista - " + nomeCurso + "</title></head><body>" + script + runScript + "</body></html>");
    win.document.close();
  };
    ...
    <div>
      <div className="header no-print">
        <div className="container header-inner">
          <a href="/" className="back-btn"><BackIcon /> Voltar</a>
          <img src="/logo.jpg" alt="Tulipa" className="header-logo" />
          <div className="header-text">
            <h1><CourseIcon /> Cursos</h1>
        ...
        {!cursoSelecionado ? (
            ...
            <div className="cursos-grid">
              {cursos.map((curso) => (
                <button key={curso.id} type="button" className="curso-card" onClick={() => setCursoSelecionado(curso)}>
                  <div className="curso-icon"><CourseIcon /></div>
                  <h3>{curso.nome}</h3>
                  <p>{curso.descricao}</p>
          ...
          <div>
            <div className="curso-header-bar">
              <button className="btn btn-secondary" onClick={() => setCursoSelecionado(null)}><BackIcon /> Trocar Curso</button>
              <div>
              ...
              <div className="curso-actions">
                {alunos.length > 0 ? (
                  <>
                    <button className="btn btn-primary" onClick={gerarPDF}><PdfIcon /> Gerar PDF</button>
                    <button className="btn" style={{background:"#25D366",color:"#fff"}} onClick={compartilharWhatsApp}><WhatsIcon /> WhatsApp</button>
                    <button className="btn btn-secondary" onClick={() => window.print()}><PrintIcon /> Imprimir</button>
                  </>
                ) : null}
              </div>
            </div>
            <div className="curso-form-card">
              <h3 style={{fontSize:"1rem",fontWeight:700,color:"#0d5e35",marginBottom:16}}>
                {editandoIdx !== null ? "Editar Aluno" : "Adicionar Aluno"}
              </h3>
            ...
            {alunos.length === 0 ? (
              ...
              <div className="curso-table-card">
                <div className="curso-table-header">
                  <ListaIcon /> Lista de Alunos ({alunos.length})
                </div>
                      ...
                      <tr key={idx}>
                        ...
                        <td>
                          <div style={{display:"flex",gap:6}}>
                            <button className="curso-action-btn edit" onClick={() => iniciarEdicao(idx)}><EditIcon /></button>
                            <button className="curso-action-btn delete" onClick={() => removerAluno(idx)}><DeleteIcon /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
