...
export default function VisBellaPage() {
  ...
  useEffect(() => {
    fetch("/api/visbella-produtos")
      .then((r) => r.json())
      .then((data) => {
        if (data.erro) throw new Error(data.erro);
        setProdutos(data);
        const cats = [...new Set(data.map((p: Produto) => p.categoria))] as string[];
        setCategorias(cats);
        setLoading(false);
      })
      .catch((e) => {
  ...
  const updateItem = (produto: Produto, qtd: number) => {
    setItens((prev) => {
      const next = { ...prev };
      if (qtd <= 0) {
        delete next[produto.id];
      } else {
        next[produto.id] = { produto, qtd };
      }
      return next;
    });
  };

  const totalQtd = Object.values(itens).reduce((s, i) => s + i.qtd, 0);
  ...
  const gerarRomaneio = () => {
    ...
    for (const [categoria, items] of Object.entries(grupos)) {
      rows.push(`<tr style="background:#fff3e0;"><td colspan="4" style="padding:8px 10px;font-weight:800;font-size:15px;color:#e65100;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e65100;">${categoria}</td></tr>`);
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
      ...
      * { margin: 0; padding: 0; box-sizing: border-box; font-family: Arial, sans-serif; }
      ...
      .h { text-align: center; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 3px solid #e65100; }
      .h h1 { font-size: 26px; color: #e65100; }
      ...
      th { background: #e65100; color: #fff; padding: 8px 10px; text-align: left; font-size: 13px; }
      .total { background: #fff3e0; font-weight: 700; }
      .total td { padding: 10px 8px; border-top: 2px solid #e65100; }
      .foot { text-align: center; margin-top: 40px; padding-top: 20px; border-top: 1px solid #ddd; color: #999; font-size: 11px; }
    </style></head><body>
      <div class="h"><h1>Romaneio - Vis Bella Brasil</h1><p>Pedido gerado em ${dataAtual}</p></div>
      <div class="info"><div><strong>Vendedor(a):</strong> ${vendedor || "Não informado"}</div><div><strong>Total itens:</strong> ${totalItens} | <strong>Qtd:</strong> ${totalQtd}</div></div>
      <table><thead><tr><th></th><th>Produto</th><th>Ref</th><th style="text-align:center;">Qtd</th></tr></thead><tbody>
        ${rows.join("")}
        <tr class="total"><td></td><td><strong>TOTAL GERAL</strong></td><td></td><td style="text-align:center;font-weight:900;font-size:20px;color:#e65100;">${totalQtd}</td></tr>
      </tbody></table>
    ...
    return (
      <div className="page-wrap">
        <div className="loading-screen">
          ...
          <div className="loading-text">Carregando produtos Vis Bella...</div>
        </div>
      </div>
    );
  }
      ...
      <div className="page-wrap">
        <div className="header no-print">
          <div className="container header-inner">
            ...
            <div className="header-text"><h1>Vis Bella Brasil</h1></div>
          </div>
        </div>
  ...
  return (
    <div className="page-wrap">
      <div className="header no-print" style={{ background: "linear-gradient(135deg, #e65100, #f57c00)" }}>
        <div className="container header-inner">
          <a href="/pedidos-fornecedores" className="back-btn">← Voltar</a>
          <div className="header-text">
            <h1>Vis Bella Brasil</h1>
      ...
      <main className="main-content">
        <div className="container">
          ...
          <div className="resumo-bar no-print">
            ...
            <div className="resumo-item">
              <span className="resumo-num">{totalQtd}</span>
              <span className="resumo-leg">qtd</span>
            </div>
      ...
      <style>{`
        ...
        .loading-spinner { width: 40px; height: 40px; border: 4px solid #e2e8f0; border-top-color: #e65100; border-radius: 50%; animation: spin 0.7s linear infinite; }
        ...
        .empty-state h2 { color: #e65100; margin-bottom: 8px; font-size: 1.2rem; }
        ...
        .vendedor-input:focus { outline: none; border-color: #e65100; box-shadow: 0 0 0 4px rgba(230,81,0,0.1); }
        ...
        .resumo-item { display: flex; flex-direction: column; align-items: center; background: #fff3e0; padding: 8px 20px; border-radius: 12px; }
        .resumo-num { font-size: 1.4rem; font-weight: 900; color: #e65100; line-height: 1.2; }
        .resumo-leg { font-size: 0.7rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.5px; }
        .btn-resumo { margin-left: auto; padding: 14px 24px; border: none; border-radius: 14px; background: #e65100; color: #fff; font-weight: 800; font-size: 1rem; font-family: inherit; white-space: nowrap; cursor: pointer; box-shadow: 0 6px 14px rgba(230,81,0,0.25); transition: all 0.15s; }
        .btn-resumo:active { transform: scale(0.97); }
        .btn-resumo:disabled { opacity: 0.4; }

        .cat-section { padding: 0 12px; margin-bottom: 24px; }
        .cat-title { font-size: 1.1rem; font-weight: 800; color: #e65100; margin-bottom: 10px; padding: 0 4px; }
        ...
        .prod-card.has-item { border: 2px solid #e65100; box-shadow: 0 4px 16px rgba(230,81,0,0.15); }
        ...
        .qtd-btn { width: 34px; height: 34px; border-radius: 8px; border: 2px solid #e2e8f0; background: #fff; cursor: pointer; font-size: 18px; font-weight: 700; display: flex; align-items: center; justify-content: center; color: #e65100; }
        .qtd-btn:active { background: #fff3e0; border-color: #e65100; }
...
function ProdutoCard({
  ...
  updateItem: (produto: Produto, qtd: number) => void;
}) {
      ...
      <div className="prod-card-inner">
        ...
        <div className="prod-body">
          ...
          <div className="qtd-row">
            <button className="qtd-btn" onClick={() => handleQtd((item?.qtd || 0) - 1)} disabled={!item}>−</button>
            <input type="number" min="0" className="qtd-input" value={item?.qtd || 0}
              onChange={(e) => handleQtd(parseInt(e.target.value) || 0)} />
            <button className="qtd-btn" onClick={() => handleQtd((item?.qtd || 0) + 1)}>+</button>
          </div>
        </div>
      </div>
    </div>
